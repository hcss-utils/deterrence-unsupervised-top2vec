...
def check_embedding_model(model):
    ...
    if model not in valid_models:
        ...
        typer.echo(f"The valid string options for 'embedding_model' are: {', '.join(valid_models)}")
        raise typer.Exit(code=1)
    return model
...
def train(
    ...
    speed: str = typer.Option(
        "learn", 
        help="How fast the model takes to train",
        callback=check_speed
    ),
    ...
    if embedding_model == "doc2vec":
        typer.echo(f"Training the model with following parameters: {embedding_model=}, {speed=}, {workers=}")
        t2v = Top2Vec(
            documents=docs, 
